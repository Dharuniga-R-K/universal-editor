  ...
  const jQuery = (function () {
    ...
    jQuery.extend({
      ...
      map(elems, callback, arg) {
        let value; let key; const ret = [];
        let i = 0;
        const { length } = elems;
        // jquery objects are treated as arrays
        const isArray = elems instanceof jQuery || length !== undefined && typeof length === 'number' && ((length > 0 && elems[0] && elems[length - 1]) || length === 0 || jQuery.isArray(elems));
 ...
 */
  ...
  jQuery.extend({
    ...
    Deferred(func) {
      const doneList = jQuery.Callbacks('once memory');
      const failList = jQuery.Callbacks('once memory');
      const progressList = jQuery.Callbacks('memory');
      let state = 'pending';
      const lists = {
        resolve: doneList,
        reject: failList,
    ...
    // Deferred helper
    when(firstParam) {
      const args = sliceDeferred.call(arguments, 0);
      let i = 0;
      const { length } = args;
      const pValues = new Array(length);
      let count = length;
      const pCount = length;
      const deferred = length <= 1 && firstParam && jQuery.isFunction(firstParam.promise)
        ? firstParam
        : jQuery.Deferred();
  ...
  jQuery.support = (function () {
    ...
    jQuery(() => {
      let container; let outer; let inner; let table; let td; let offsetSupport;
      let conMarginTop; let ptlm; let vb; let style; let html;
  ...
  jQuery.extend({
    ...
    data(elem, name, data, pvt /* Internal Use Only */) {
      ...
      let privateCache; let thisCache; let ret;
    ...
    removeData(elem, name, pvt /* Internal Use Only */) {
      ...
      // See jQuery.data for more information
      const id = isNode ? elem[internalKey] : internalKey;
  ...
  function handleQueueMarkDefer(elem, type, src) {
    ...
    const queueDataKey = `${type}queue`;
    const markDataKey = `${type}mark`;
    const defer = jQuery._data(elem, deferDataKey);
    if (defer
  ...
  jQuery.extend({
    ...
    _unmark(force, elem, type) {
      ...
      if (elem) {
        type = type || 'fx';
        const key = `${type}mark`;
        const count = force ? 0 : ((jQuery._data(elem, key) || 1) - 1);
        if (count) {
  ...
  const { getSetAttribute } = jQuery.support;
  let nodeHook; let boolHook; let
    fixSpecified;
  ...
  jQuery.fn.extend({
    ...
    addClass(value) {
      let classNames; let i; let l; let elem;
      let setClass; let c; let
        cl;
    ...
    removeClass(value) {
      let classNames; let i; let l; let elem; let className; let c; let
        cl;
  ...
  jQuery.extend({
    valHooks: {
      ...
      select: {
        get(elem) {
          let value; let i; let max; let option;
    ...
    removeAttr(elem, value) {
      let propName; let attrNames; let name; let l;
      let i = 0;
  ...
  boolHook = {
    get(elem, name) {
      ...
      let attrNode;
      const property = jQuery.prop(elem, name);
  ...
  const hoverHack = function (events) {
 ...
 */
  jQuery.event = {
    ...
    add(elem, types, handler, data, selector) {
      let elemData; let eventHandle; let events;
      let t; let tns; let type; let namespaces; let handleObj;
      let handleObjIn; let quick; let handlers; let
        special;
    ...
    remove(elem, types, handler, selector, mappedTypes) {
      ...
      let t; let tns; let type; let origType; let namespaces; let origCount;
      let j; let events; let special; let handle; let eventType; let
        handleObj;
    ...
    trigger(event, data, elem, onlyHandlers) {
      ...
      let namespaces = [];
      let cache; let exclusive; let i; let cur; let old; let ontype; let special; let handle; let eventPath; let
        bubbleType;
    ...
    dispatch(event) {
      ...
      const run_all = !event.exclusive && !event.namespace;
      const handlerQueue = [];
      let i; let j; let cur; let jqcur; let ret; let selMatch; let matched; let matches; let handleObj; let sel; let
        related;
    ...
    fix(event) {
      ...
      const fixHook = jQuery.event.fixHooks[event.type] || {};
      const copy = fixHook.props ? this.props.concat(fixHook.props) : this.props;
  ...
  jQuery.fn.extend({
    ...
    toggle(fn) {
      ...
      const args = arguments;
      const guid = fn.guid || jQuery.guid++;
      let i = 0;
 ...
 */
  (function () {
    const chunker = /((?:\((?:\([^()]+\)|[^()]+)+\)|\[(?:\[[^\[\]]*\]|['"][^'"]*['"]|[^\[\]'"]+)+\]|\\.|[^ >+~,(\[\\]+)+|[>+~])(\s*,\s*)?((?:.|\r|\n)*)/g;
    const expando = `sizcache${(`${Math.random()}`).replace('.', '')}`;
    let done = 0;
    const { toString } = Object.prototype;
    ...
    Sizzle.find = function (expr, context, isXML) {
      let set; let i; let len; let match; let type; let
        left;
    ...
    Sizzle.filter = function (expr, set, inplace, not) {
      ...
      let type; let found; let item; let filter; let left;
      let i; let pass;
      let old = expr;
      let result = [];
      let curLoop = set;
 ...
 */
    ...
    var Expr = Sizzle.selectors = {
      ...
      relative: {
        '+': function (checkSet, part) {
          ...
          const isTag = isPartStr && !rNonWord.test(part);
      ...
      filters: {
        ...
        text(elem) {
          const attr = elem.getAttribute('type'); const
            { type } = elem;
      ...
      filter: {
        ...
        CHILD(elem, match) {
          let first; let last;
          let doneName; let parent; let cache;
          let count; let diff;
          const type = match[1];
          let node = elem;
        ...
        Sizzle = function (query, context, extra, seed) {
          ...
          if (!seed && !Sizzle.isXML(context)) {
            ...
            if (context.nodeType === 9) {
              ...
              let nid = old || id;
              const hasParent = context.parentNode;
              const relativeHierarchySelector = /^\s*[+~]/.test(query);
    ...
    (function () {
      ...
      if (matches) {
        ...
        const disconnectedMatch = !matches.call(document.createElement('div'), 'div');
  ...
  jQuery.fn.extend({
    ...
    closest(selectors, context) {
        ...
        while (cur && cur.ownerDocument && cur !== context) {
          for (i = 0; i < selectors.length; i++) {
            if (jQuery(cur).is(selectors[i])) {
              ret.push({ selector: selectors[i], elem: cur, level });
            }
          }
      ...
      // String
      const pos = POS.test(selectors) || typeof selectors !== 'string'
  ...
  jQuery.buildFragment = function (args, nodes, scripts) {
    let fragment; let cacheable; let cacheresults; let doc;
    const first = args[0];
  ...
  jQuery.extend({
    clone(elem, dataAndEvents, deepDataAndEvents) {
      ...
      const clone = jQuery.support.html5Clone || !rnoshimcache.test(`<${elem.nodeName}`)
        ? elem.cloneNode(true)
        : shimCloneNode(elem);
  ...
  jQuery.extend({
    ...
    style(elem, name, value, extra) {
      ...
      let ret; let type; const origName = jQuery.camelCase(name);
  ...
  if (document.documentElement.currentStyle) {
    currentStyle = function (elem, name) {
      let left; let rsLeft; let uncomputed;
  ...
  const rinput = /^(?:color|date|datetime|datetime-local|email|hidden|month|number|password|range|search|tel|text|time|url|week)$/i;
  ...
  function addToPrefiltersOrTransports(structure) {
    ...
    return function (dataTypeExpression, func) {
      ...
      if (jQuery.isFunction(func)) {
        const dataTypes = dataTypeExpression.toLowerCase().split(rspacesAjax);
        let i = 0;
        const { length } = dataTypes;
        let dataType;
        let list;
        let placeBefore;
  ...
  function inspectPrefiltersOrTransports(
    ...
    const list = structure[dataType];
    let i = 0;
    const length = list ? list.length : 0;
    const executeOnly = (structure === prefilters);
    let selection;
  ...
  jQuery.extend({
    ...
    ajax(url, options) {
      ...
      const // Create the final options object
      ...
      const globalEventContext = callbackContext !== s
      ...
      function done(status, nativeStatusText, responses, headers) {
        ...
        const response = responses ? ajaxHandleResponses(s, jqXHR, responses) : undefined;
        let lastModified;
        let etag;
 ...
 */
  ...
  jQuery.ajaxPrefilter('json jsonp', (s, originalSettings, jqXHR) => {
    const inspectData = s.contentType === 'application/x-www-form-urlencoded'
    ...
    if (s.dataTypes[0] === 'jsonp'
      ...
      const jsonpCallback = s.jsonpCallback =				jQuery.isFunction(s.jsonpCallback) ? s.jsonpCallback() : s.jsonpCallback;
  ...
  const // #5280: Internet Explorer will keep connections alive if we don't abort on unload
  ...
  const elemdisplay = {};
  let iframe; let iframeDoc;
  const rfxtypes = /^(?:toggle|show|hide)$/;
  const rfxnum = /^([+\-]=)?([\d+.\-]+)([a-z%]*)$/i;
  let timerId;
  const fxAttrs = [
    // height animations
  ...
  jQuery.fn.extend({
    ...
    animate(prop, speed, easing, callback) {
      ...
      function doAnimation() {
        ...
        const opt = jQuery.extend({}, optall);
        const isElement = this.nodeType === 1;
        const hidden = isElement && jQuery(this).is(':hidden');
        let name; let val; let p; let e;
        let parts; let start; let end; let unit;
        let method;
  ...
  jQuery.fx.prototype = {
    ...
    cur() {
      ...
      let parsed;
      const r = jQuery.css(this.elem, this.prop);
    ...
    step(gotoEnd) {
      let p; let n; let complete;
      const t = fxNow || createFxNow();
      let done = true;
      const { elem } = this;
      const { options } = this;
  ...
  function defaultDisplay(nodeName) {
    if (!elemdisplay[nodeName]) {
      const { body } = document;
      let elem = jQuery(`<${nodeName}>`).appendTo(body);
  ...
  jQuery.offset = {
    ...
    setOffset(elem, options, i) {
      ...
      const calculatePosition = (position === 'absolute' || position === 'fixed') && jQuery.inArray('auto', [curCSSTop, curCSSLeft]) > -1;
