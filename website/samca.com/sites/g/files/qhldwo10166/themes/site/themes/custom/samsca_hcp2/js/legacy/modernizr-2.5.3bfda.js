...
window.Modernizr = (function (window, document, undefined) {
  ...
  const testMediaQuery = function (mq) {
    ...
    injectElementWithStyles(`@media ${mq} { #${mod} { position: absolute; } }`, (node) => {
  ...
  const _hasOwnProperty = ({}).hasOwnProperty; let
    hasOwnProperty;
  ...
  /**
     ...
     */
  /* >>testBundle */
  const testBundle = (function (styles, tests) {
    const style = styles.join('');
    let len = tests.length;

    injectElementWithStyles(style, (node, rule) => {
      ...
      const cssText = style ? (style.cssRules && style.cssRules[0] ? style.cssRules[0].cssText : style.cssText || '') : '';
      const children = node.childNodes; const
        hash = {};
  ...
  tests.websockets = function () {
    for (let i = -1, len = cssomPrefixes.length; ++i < len;) {
  ...
  (function (window, document) {
    ...
    /**
         ...
         */
    const reSkip = /^<|^(?:button|form|map|select|textarea)$/i;
    ...
    function shivMethods(ownerDocument) {
      const cache = {};
      const docCreateElement = ownerDocument.createElement;
      const docCreateFragment = ownerDocument.createDocumentFragment;
      const frag = docCreateFragment();
      ...
      ownerDocument.createElement = function (nodeName) {
        ...
        const node = (cache[nodeName] || (cache[nodeName] = docCreateElement(nodeName))).cloneNode();
