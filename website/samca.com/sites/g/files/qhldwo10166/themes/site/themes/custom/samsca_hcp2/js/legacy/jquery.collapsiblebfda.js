...
(function ($) {
  $.fn.jcollapse = function (cmd, arg) {
    ...
    if (typeof cmd === 'string') {
      return $.fn.jcollapse.dispatcher[cmd](this, arg);
    }

    // return the command dispatcher
    return $.fn.jcollapse.dispatcher._create(this, cmd);
  };

  // create the command dispatcher
  $.fn.jcollapse.dispatcher = {
  ...
  function createCollapsible(obj, options) {
    ...
    const opts = $.extend({}, $.fn.jcollapse.defaults, options);
    ...
    obj.each(
      function () {
        ...
        if (!useCookies(opts)) {
          ...
          if (issetCookie(opts)) {
            const cookieIndex = inCookie(id, opts);
  ...
  function appendCookie(value, opts) {
    ...
    // get the cookie
    const cookie = decodeURIComponent($.cookie(opts.cookieName));
  ...
  function unsetCookieId(value, opts) {
    ...
    const cookieIndex = inCookie(value, opts);
  ...
  function inCookie(value, opts) {
    ...
    const cookieIndex = $.inArray(value, cookieArray);
  ...
  function inDefaultOpen(id, opts) {
    ...
    const defaultOpen = getDefaultOpen(opts);

    // is it in the default open array
    const index = $.inArray(id, defaultOpen);
